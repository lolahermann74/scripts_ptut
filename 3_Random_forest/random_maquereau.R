# Random forest pour le maquereau
...
print(rf_model)
importance_cf <- varimp(rf_model, conditional = FALSE)
...
if(min(donnees$Nbr) > 0) {
  seuil_coupure_poisson <- mean(donnees$Nbr)
...
plot(roc_obj, 
     ...
     col = "#004D40", lwd = 3)
points(resultat_seuil$specificity, resultat_seuil$sensitivity, # Ajout du point seuil sur le graphique
...
donnees$Groupe_Seuil <- ifelse(donnees[[ma_variable]] > valeur_seuil, 
...
# On récupère les moyennes pour les comparer
...
ggplot(donnees, aes(x = Groupe_Seuil, y = Nbr, fill = Groupe_Seuil)) +
  ...
  coord_cartesian(ylim = c(0, quantile(donnees$Nbr, 0.95) * 1.5)) + 
  scale_fill_manual(values = c("#56B4E9", "#E69F00")) +
  theme_minimal() +
  labs(title = paste("Effet de", ma_variable, "sur l'abondance de maquereaux"),
