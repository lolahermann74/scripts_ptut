...
idx_transect <- st_nearest_feature(coords_maquereau_sf, zone_maq_l93)

# Ajout des infos du transect et calcul de la distance d'erreur (en mètres)
coords_maquereau_sf <- coords_maquereau_sf %>%
  mutate(
...
ggplot() +
  ...
  geom_sf(data = coords_maquereau_sf, aes(color = as.factor(transect_id)), size = 2) +
