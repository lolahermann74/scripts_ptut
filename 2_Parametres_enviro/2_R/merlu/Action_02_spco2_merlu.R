# ============================================================
# Extraction carbon (Copernicus NetCDF 4D: lon/lat/depth/time) sur captures merlu
...
# CHEMINS
# ---------------------------
merlu_csv <- "G:/PTUT_AVIZONS/R/data/merlu.csv"
...
# ---------------------------
process_one_year <- function(y, merlu_y) {
  
  message("\n===== Année ", y, " =====")
  message("N captures : ", nrow(merlu_y))
  
  built <- build_year_brick(
    y = y,
    capture_dates_posix = merlu_y$date_posix,
    nc_dir = nc_dir,
  ...
  coords <- as.matrix(merlu_y[, .(get(lon_col), get(lat_col))])
  colnames(coords) <- c("lon","lat")
  
  merlu_y[, cell := cellFromXY(sal_brick, coords)]
  merlu_y[, depth_used_m := depth_used]
  
  # ------------------------------------------------------------
  # Ajouter coords Copernicus du pixel le plus proche + distance
  # ------------------------------------------------------------
  # xyFromCell renvoie (lon, lat) au centre de la cellule
  cop_xy <- xyFromCell(sal_brick, merlu_y$cell)
  
  merlu_y[, long_cop := cop_xy[, 1]]
  merlu_y[, lat_cop  := cop_xy[, 2]]
  
  # Distance (m) entre point capture et centre du pixel (haversine)
  # (coords capture = lon_col/lat_col)
  merlu_y[, dist_m := geosphere::distHaversine(
    cbind(get(lon_col), get(lat_col)),
    cbind(long_cop, lat_cop)
  )]
  
  unique_cells <- unique(na.omit(merlu_y$cell))
  ...
  for (prefix in c("m1","m3","m6","y1","y2")) {
    for (suf in c("mean","median","q1","q3","sd","n")) {
      merlu_y[, (paste0(prefix, "_", suf)) := if (suf == "n") NA_integer_ else NA_real_]
    }
  }
  
  for (i in seq_len(nrow(merlu_y))) {
    cell_i <- merlu_y$cell[i]
    if (is.na(cell_i)) next
    
    d0  <- as.Date(merlu_y$date_posix[i])
    ...
    merlu_y$sal_day_t0[i]      <- if (length(v0) == 1) v0 else NA_real_
    merlu_y$sal_day_tminus1[i] <- if (length(v1) == 1) v1 else NA_real_
    
    # --- semaine avant : t-7 à t-1 ---
    w_vals <- dt_day[day >= (d0 - 7) & day <= (d0 - 1), spco2_day]
    n_av <- sum(!is.na(w_vals))
    merlu_y$n_days_available[i] <- n_av
    merlu_y$sal_week_mean[i]    <- if (n_av > 0) mean(w_vals, na.rm = TRUE) else NA_real_
    merlu_y$sal_week_median[i]  <- if (n_av > 0) median(w_vals, na.rm = TRUE) else NA_real_
    merlu_y$sal_week_sd[i]      <- if (n_av > 0) sd(w_vals, na.rm = TRUE) else NA_real_
    ...
    merlu_y$m1_mean[i] <- st$mean; merlu_y$m1_median[i] <- st$median;
    merlu_y$m1_q1[i] <- st$q1; merlu_y$m1_q3[i] <- st$q3; merlu_y$m1_sd[i] <- st$sd; merlu_y$m1_n[i] <- st$n
    
    st <- stats_vec(m3_vals)
    merlu_y$m3_mean[i] <- st$mean; merlu_y$m3_median[i] <- st$median;
    merlu_y$m3_q1[i] <- st$q1; merlu_y$m3_q3[i] <- st$q3; merlu_y$m3_sd[i] <- st$sd; merlu_y$m3_n[i] <- st$n
    
    st <- stats_vec(m6_vals)
    merlu_y$m6_mean[i] <- st$mean; merlu_y$m6_median[i] <- st$median;
    merlu_y$m6_q1[i] <- st$q1; merlu_y$m6_q3[i] <- st$q3; merlu_y$m6_sd[i] <- st$sd; merlu_y$m6_n[i] <- st$n
    ...
    merlu_y$y1_mean[i] <- st$mean; merlu_y$y1_median[i] <- st$median;
    merlu_y$y1_q1[i] <- st$q1; merlu_y$y1_q3[i] <- st$q3; merlu_y$y1_sd[i] <- st$sd; merlu_y$y1_n[i] <- st$n
    
    st <- stats_vec(y2_vals)
    merlu_y$y2_mean[i] <- st$mean; merlu_y$y2_median[i] <- st$median;
    merlu_y$y2_q1[i] <- st$q1; merlu_y$y2_q3[i] <- st$q3; merlu_y$y2_sd[i] <- st$sd; merlu_y$y2_n[i] <- st$n
  }
...
merlu <- fread(merlu_csv)

# date -> POSIXct (format type "2024-10-23 13:21:00")
merlu[, date_posix := as.POSIXct(get(date_col), tz = "UTC")]

# fallback format FR si besoin
if (any(is.na(merlu$date_posix))) {
  merlu[, date_posix := suppressWarnings(as.POSIXct(get(date_col),
                                                    format = "%d/%m/%Y %H:%M:%S", tz="UTC"))]
}

if (any(is.na(merlu$date_posix))) {
  stop("Dates non converties. Exemple brut: ", merlu[[date_col]][which(is.na(merlu$date_posix))[1]])
}

merlu[, year := year(date_posix)]
years <- sort(unique(merlu$year))
message("Années détectées : ", paste(years, collapse = ", "))

for (y in years) {
  merlu_y <- merlu[year == y]
  res_y <- process_one_year(y, merlu_y)
  out_file <- file.path(out_dir, sprintf("spco2_merlu_%d.csv", y))
