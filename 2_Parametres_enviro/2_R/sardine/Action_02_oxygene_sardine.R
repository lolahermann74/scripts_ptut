# Extraction oxygène (Copernicus NetCDF 4D: lon/lat/depth/time) sur captures sardine
...
nc_dir    <- "G:/PTUT_AVIZONS/R/data/oxygene"
out_dir   <- "G:/PTUT_AVIZONS/R/output"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

nc_pattern <- "oxygene_ibi_%d.nc"
...
# ---------------------------
# Ouvre un nc et renvoie : brick oxygène au niveau depth_idx + time POSIXct
# ---------------------------
open_oxygene_nc <- function(nc_file, varname, time_varname, depth_varname, target_depth) {
...
build_year_brick <- function(y, capture_dates_posix, nc_dir, nc_pattern,
  ...
  for (yy in years_to_load) {
    ...
    o <- open_oxygene_nc(f, varname, time_varname, depth_varname, target_depth)
...
process_one_year <- function(y, sardine_y) {
  ...
  for (cid in rownames(ts_mat)) {
    v <- ts_mat[cid, ]
    o2_day <- vapply(idx_by_day, function(idxs) mean(v[idxs], na.rm = TRUE), numeric(1))
    dt <- data.table(day = as.Date(names(o2_day)), o2_day = as.numeric(o2_day))
  ...
  for (i in seq_len(nrow(sardine_y))) {
    ...
    # --- t0 / t-1 (journalier) ---
    v0 <- dt_day[day == d0, o2_day]
    v1 <- dt_day[day == (d0 - 1), o2_day]
    ...
    w_vals <- dt_day[day >= (d0 - 7) & day <= (d0 - 1), o2_day]
    ...
    m1_vals <- dt_day[day >= m1_start & day <= m1_end, o2_day]
    m3_vals <- dt_day[day >= m3_start & day <= m3_end, o2_day]
    m6_vals <- dt_day[day >= m6_start & day <= m6_end, o2_day]
    ...
    y1_vals <- dt_day[day >= y1_start & day <= y1_end, o2_day]
    y2_vals <- dt_day[day >= y2_start & day <= y2_end, o2_day]
...
for (y in years) {
  ...
  out_file <- file.path(out_dir, sprintf("oxygene_sardine_%d.csv", y))
