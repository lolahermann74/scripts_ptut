...
process_one_year <- function(y, sardine_y) {
  ...
  for (cid in rownames(ts_mat)) {
    v <- ts_mat[cid, ]
    ph_day <- vapply(idx_by_day, function(idxs) mean(v[idxs], na.rm = TRUE), numeric(1))
    dt <- data.table(day = as.Date(names(ph_day)), ph_day = as.numeric(ph_day))
  ...
  for (i in seq_len(nrow(sardine_y))) {
    ...
    # --- t0 / t-1 (journalier) ---
    v0 <- dt_day[day == d0, ph_day]
    v1 <- dt_day[day == (d0 - 1), ph_day]
    ...
    w_vals <- dt_day[day >= (d0 - 7) & day <= (d0 - 1), ph_day]
    ...
    m1_vals <- dt_day[day >= m1_start & day <= m1_end, ph_day]
    m3_vals <- dt_day[day >= m3_start & day <= m3_end, ph_day]
    m6_vals <- dt_day[day >= m6_start & day <= m6_end, ph_day]
    ...
    y1_vals <- dt_day[day >= y1_start & day <= y1_end, ph_day]
    y2_vals <- dt_day[day >= y2_start & day <= y2_end, ph_day]
...
for (y in years) {
  ...
  out_file <- file.path(out_dir, sprintf("ph_sardine_%d.csv", y))
