# Extraction salinité (Copernicus NetCDF 4D: lon/lat/depth/time) sur captures sardine
# + Features semaine avant + features mois/années avant
# Variable salinité : so
# Auteur : Hermann Lola
...
# ---------------------------
# Ouvre un nc et renvoie : brick salinité au niveau depth_idx + time POSIXct
# ---------------------------
...
process_one_year <- function(y, sardine_y) {
  ...
  for (i in seq_len(nrow(sardine_y))) {
    ...
    m1_vals <- dt_day[day >= m1_start & day <= m1_end, so_day]
    m3_vals <- dt_day[day >= m3_start & day <= m3_end, so_day]
    m6_vals <- dt_day[day >= m6_start & day <= m6_end, so_day]
    ...
    y1_vals <- dt_day[day >= y1_start & day <= y1_end, so_day]
    y2_vals <- dt_day[day >= y2_start & day <= y2_end, so_day]
...
for (y in years) {
  ...
  out_file <- file.path(out_dir, sprintf("salinite_sardine_%d.csv", y))
