# Extraction nitrate (Copernicus NetCDF 4D: lon/lat/depth/time) sur captures sardine
# + Features semaine avant + features mois/années avant
# Variable nitrate : no3
# Auteur : Hermann Lola
...
nc_dir    <- "G:/PTUT_AVIZONS/R/data/nutriments"
out_dir   <- "G:/PTUT_AVIZONS/R/output"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

nc_pattern <- "nutriments_ibi_%d.nc"
...
# ---------------------------
# Ouvre un nc et renvoie : brick nitrate au niveau depth_idx + time POSIXct
# ---------------------------
open_nitrate_nc <- function(nc_file, varname, time_varname, depth_varname, target_depth) {
...
build_year_brick <- function(y, capture_dates_posix, nc_dir, nc_pattern,
  ...
  for (yy in years_to_load) {
    ...
    o <- open_nitrate_nc(f, varname, time_varname, depth_varname, target_depth)
...
process_one_year <- function(y, sardine_y) {
  ...
  for (cid in rownames(ts_mat)) {
    v <- ts_mat[cid, ]
    no3_day <- vapply(idx_by_day, function(idxs) mean(v[idxs], na.rm = TRUE), numeric(1))
    dt <- data.table(day = as.Date(names(no3_day)), no3_day = as.numeric(no3_day))
  ...
  for (i in seq_len(nrow(sardine_y))) {
    ...
    # --- t0 / t-1 (journalier) ---
    v0 <- dt_day[day == d0, no3_day]
    v1 <- dt_day[day == (d0 - 1), no3_day]
    ...
    w_vals <- dt_day[day >= (d0 - 7) & day <= (d0 - 1), no3_day]
    ...
    m1_vals <- dt_day[day >= m1_start & day <= m1_end, no3_day]
    m3_vals <- dt_day[day >= m3_start & day <= m3_end, no3_day]
    m6_vals <- dt_day[day >= m6_start & day <= m6_end, no3_day]
    ...
    y1_vals <- dt_day[day >= y1_start & day <= y1_end, no3_day]
    y2_vals <- dt_day[day >= y2_start & day <= y2_end, no3_day]
...
for (y in years) {
  ...
  out_file <- file.path(out_dir, sprintf("nitrate_sardine_%d.csv", y))
