# Extraction carbon (Copernicus NetCDF 4D: lon/lat/depth/time) sur captures sardine
# + Features semaine avant + features mois/années avant
# Variable carbon : spco2
# Auteur : Hermann Lola
...
# CHEMINS
# ---------------------------
sardine_csv <- "G:/PTUT_AVIZONS/R/data/sardine.csv"
nc_dir    <- "G:/PTUT_AVIZONS/R/data/carbon"
out_dir   <- "G:/PTUT_AVIZONS/R/output"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

nc_pattern <- "carbon_ibi_%d.nc"
...
# ---------------------------
# Ouvre un nc et renvoie : brick carbon au niveau depth_idx + time POSIXct
# ---------------------------
open_carbon_nc <- function(nc_file, varname, time_varname, depth_varname, target_depth) {
...
build_year_brick <- function(y, capture_dates_posix, nc_dir, nc_pattern,
  ...
  for (yy in years_to_load) {
    ...
    o <- open_carbon_nc(f, varname, time_varname, depth_varname, target_depth)
...
# ---------------------------
process_one_year <- function(y, sardine_y) {
  
  message("\n===== Année ", y, " =====")
  message("N captures : ", nrow(sardine_y))
  
  built <- build_year_brick(
    y = y,
    capture_dates_posix = sardine_y$date_posix,
    nc_dir = nc_dir,
  ...
  coords <- as.matrix(sardine_y[, .(get(lon_col), get(lat_col))])
  colnames(coords) <- c("lon","lat")
  
  sardine_y[, cell := cellFromXY(sal_brick, coords)]
  sardine_y[, depth_used_m := depth_used]
  
  # ------------------------------------------------------------
  # Ajouter coords Copernicus du pixel le plus proche + distance
  # ------------------------------------------------------------
  # xyFromCell renvoie (lon, lat) au centre de la cellule
  cop_xy <- xyFromCell(sal_brick, sardine_y$cell)
  
  sardine_y[, long_cop := cop_xy[, 1]]
  sardine_y[, lat_cop  := cop_xy[, 2]]
  
  # Distance (m) entre point capture et centre du pixel (haversine)
  # (coords capture = lon_col/lat_col)
  sardine_y[, dist_m := geosphere::distHaversine(
    cbind(get(lon_col), get(lat_col)),
    cbind(long_cop, lat_cop)
  )]
  
  unique_cells <- unique(na.omit(sardine_y$cell))
  ...
  for (cid in rownames(ts_mat)) {
    v <- ts_mat[cid, ]
    spco2_day <- vapply(idx_by_day, function(idxs) mean(v[idxs], na.rm = TRUE), numeric(1))
    dt <- data.table(day = as.Date(names(spco2_day)), spco2_day = as.numeric(spco2_day))
  ...
  for (prefix in c("m1","m3","m6","y1","y2")) {
    for (suf in c("mean","median","q1","q3","sd","n")) {
      sardine_y[, (paste0(prefix, "_", suf)) := if (suf == "n") NA_integer_ else NA_real_]
    }
  }
  
  for (i in seq_len(nrow(sardine_y))) {
    cell_i <- sardine_y$cell[i]
    if (is.na(cell_i)) next
    
    d0  <- as.Date(sardine_y$date_posix[i])
    cid <- as.character(cell_i)
    dt_day <- get(cid, envir = daily_env, inherits = FALSE)
    
    # --- t0 / t-1 (journalier) ---
    v0 <- dt_day[day == d0, spco2_day]
    v1 <- dt_day[day == (d0 - 1), spco2_day]
    sardine_y$sal_day_t0[i]      <- if (length(v0) == 1) v0 else NA_real_
    sardine_y$sal_day_tminus1[i] <- if (length(v1) == 1) v1 else NA_real_
    
    # --- semaine avant : t-7 à t-1 ---
    w_vals <- dt_day[day >= (d0 - 7) & day <= (d0 - 1), spco2_day]
    n_av <- sum(!is.na(w_vals))
    sardine_y$n_days_available[i] <- n_av
    sardine_y$sal_week_mean[i]    <- if (n_av > 0) mean(w_vals, na.rm = TRUE) else NA_real_
    sardine_y$sal_week_median[i]  <- if (n_av > 0) median(w_vals, na.rm = TRUE) else NA_real_
    sardine_y$sal_week_sd[i]      <- if (n_av > 0) sd(w_vals, na.rm = TRUE) else NA_real_
    ...
    m1_vals <- dt_day[day >= m1_start & day <= m1_end, spco2_day]
    m3_vals <- dt_day[day >= m3_start & day <= m3_end, spco2_day]
    m6_vals <- dt_day[day >= m6_start & day <= m6_end, spco2_day]
    
    st <- stats_vec(m1_vals)
    sardine_y$m1_mean[i] <- st$mean; sardine_y$m1_median[i] <- st$median;
    sardine_y$m1_q1[i] <- st$q1; sardine_y$m1_q3[i] <- st$q3; sardine_y$m1_sd[i] <- st$sd; sardine_y$m1_n[i] <- st$n
    
    st <- stats_vec(m3_vals)
    sardine_y$m3_mean[i] <- st$mean; sardine_y$m3_median[i] <- st$median;
    sardine_y$m3_q1[i] <- st$q1; sardine_y$m3_q3[i] <- st$q3; sardine_y$m3_sd[i] <- st$sd; sardine_y$m3_n[i] <- st$n
    
    st <- stats_vec(m6_vals)
    sardine_y$m6_mean[i] <- st$mean; sardine_y$m6_median[i] <- st$median;
    sardine_y$m6_q1[i] <- st$q1; sardine_y$m6_q3[i] <- st$q3; sardine_y$m6_sd[i] <- st$sd; sardine_y$m6_n[i] <- st$n
    ...
    y1_vals <- dt_day[day >= y1_start & day <= y1_end, spco2_day]
    y2_vals <- dt_day[day >= y2_start & day <= y2_end, spco2_day]
    
    st <- stats_vec(y1_vals)
    sardine_y$y1_mean[i] <- st$mean; sardine_y$y1_median[i] <- st$median;
    sardine_y$y1_q1[i] <- st$q1; sardine_y$y1_q3[i] <- st$q3; sardine_y$y1_sd[i] <- st$sd; sardine_y$y1_n[i] <- st$n
    
    st <- stats_vec(y2_vals)
    sardine_y$y2_mean[i] <- st$mean; sardine_y$y2_median[i] <- st$median;
    sardine_y$y2_q1[i] <- st$q1; sardine_y$y2_q3[i] <- st$q3; sardine_y$y2_sd[i] <- st$sd; sardine_y$y2_n[i] <- st$n
  }
...
sardine <- fread(sardine_csv)

# date -> POSIXct (format type "2024-10-23 13:21:00")
sardine[, date_posix := as.POSIXct(get(date_col), tz = "UTC")]

# fallback format FR si besoin
if (any(is.na(sardine$date_posix))) {
  sardine[, date_posix := suppressWarnings(as.POSIXct(get(date_col),
                                                        format = "%d/%m/%Y %H:%M:%S", tz="UTC"))]
}

if (any(is.na(sardine$date_posix))) {
  stop("Dates non converties. Exemple brut: ", sardine[[date_col]][which(is.na(sardine$date_posix))[1]])
}

sardine[, year := year(date_posix)]
years <- sort(unique(sardine$year))
message("Années détectées : ", paste(years, collapse = ", "))

for (y in years) {
  sardine_y <- sardine[year == y]
  res_y <- process_one_year(y, sardine_y)
  out_file <- file.path(out_dir, sprintf("spco2_sardine_%d.csv", y))
