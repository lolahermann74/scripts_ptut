# ============================================================
# Extraction carbon (Copernicus NetCDF 4D: lon/lat/depth/time) sur captures maquereau
...
process_one_year <- function(y, maquereau_y) {
  ...
  maquereau_y[, cell := cellFromXY(sal_brick, coords)]
  ...
  cop_xy <- xyFromCell(sal_brick, maquereau_y$cell)
  
  maquereau_y[, long_cop := cop_xy[, 1]]
  maquereau_y[, lat_cop  := cop_xy[, 2]]
  
  # Distance (m) entre point capture et centre du pixel (haversine)
  # (coords capture = lon_col/lat_col)
  maquereau_y[, dist_m := geosphere::distHaversine(
    cbind(get(lon_col), get(lat_col)),
    cbind(long_cop, lat_cop)
  )]
  
  unique_cells <- unique(na.omit(maquereau_y$cell))
  ...
  for (i in seq_len(nrow(maquereau_y))) {
    ...
    maquereau_y$sal_week_mean[i]    <- if (n_av > 0) mean(w_vals, na.rm = TRUE) else NA_real_
    maquereau_y$sal_week_median[i]  <- if (n_av > 0) median(w_vals, na.rm = TRUE) else NA_real_
    maquereau_y$sal_week_sd[i]      <- if (n_av > 0) sd(w_vals, na.rm = TRUE) else NA_real_
...
for (y in years) {
  ...
  out_file <- file.path(out_dir, sprintf("spco2_maquereau_%d.csv", y))
