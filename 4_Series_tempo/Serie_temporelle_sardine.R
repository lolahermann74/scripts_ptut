# Série temporelle de la température (spco2_y2_sd) par transect pour la sardine
...
# datasets
sardine_enviro_10fev26_398 <- read.csv("sardine_enviro_10fev26_398.csv", na = "NA")
WP1_indiv_trie <- read.csv("WP1_indiv_trie.csv", na = "NA")
sardine_with_transects_final <- read.csv("sardine_with_transects_final.csv", na = "NA")
...
WP1_sardine <- WP1 %>%
  filter(Nom_Scientifique == "Sardina pilchardus",
         Campagne == "PELGAS" )

transect_sardine <- sardine_with_transects_final

# Vérification rapide des données
head(env_sardine)
head(transect_sardine)

# Rajouter la colonne transect id au jeu de données env_sardine ####
...
env_sardine_transect <- env_sardine %>%
  left_join(ref_transect, by = c("Code_Station", "Long", "Lat"))
...
# Garder la variable ressortant du random forest sal_m6_q3####
...
ts_transect <- ts(mat, start = min(env_transect_wide_clean$Annee), frequency = 1)
...
# série temporelle annuelle
ts_transect <- ts(mat, start = min(env_transect_wide_1$Annee), frequency = 1)
...
env_long <- env_transect_wide_1 %>%
  pivot_longer(-Annee, names_to = "transect_id", values_to = "sal_m6_q3") %>%
  drop_na(sal_m6_q3)
...
ggplot(env_long, aes(x = Annee, y = sal_m6_q3, group = transect_id, colour = transect_id)) +
  geom_line(linewidth = 0.7, alpha = 0.8) +
  geom_text(data = labels_start, aes(label = transect_id), hjust = 1.1, # labels début
...
#Amélioration
env_long2 <- env_long %>%
  mutate(base = as.numeric(str_extract(transect_id, "^\\d+")),
         lat_score = case_when(
           base %in% c(28, 29) ~ 400 + base, TRUE ~ base)) %>%
  group_by(transect_id) %>%
